...
Tursa runs on the Cray Linux Environment (a version of SUSE Linux),
...
Tursa supports common revision control software such as `git`.
...
There are three different compiler environments available on Tursa:
...
Executables on Tursa link dynamically, and the Cray Programming
...
!!! warning
    Intel compilers are not available on Tursa.
...
!!! bug
    The `gcc/8.1.0` module is available on Tursa but cannot be used as the
...
passing interface which is specifically optimised for the Tursa
...
Linking to libraries is performed dynamically on Tursa. One can use
...
!!! tip
    More information on the different software libraries on Tursa can
...
```
auser@login02:/work/t01/t01/auser> module restore -s PrgEnv-gnu
auser@login02:/work/t01/t01/auser> module load cray-fftw
auser@login02:/work/t01/t01/auser> module load cray-netcdf
auser@login02:/work/t01/t01/auser> module load cray-netcdf-hdf5parallel
auser@login02:/work/t01/t01/auser> module list
Currently Loaded Modulefiles:
 ...
 5) libfabric/1.11.0.0.233(default)  13) /work/y07/shared/tursa-modules/modulefiles-cse/epcc-setup-env  
...
```
auser@login02:/work/t01/t01/auser> module load cpe/21.03
...
Currently Loaded Modulefiles:
 ...
 7) /work/y07/shared/tursa-modules/modulefiles-cse/epcc-setup-env  15) gcc/9.3.0                         
...
```
auser@login02:/work/t01/t01/auser> export LD_LIBRARY_PATH=$CRAY_LD_LIBRARY_PATH:$LD_LIBRARY_PATH
```
...
!!! important
    ...
    by the Tursa service will need to be loaded manually after you have
...
!!! note
    ...
    out and then log back in to Tursa.
...
### Available HPE Cray Programming Environment releases on Tursa

Tursa currently has the following HPE Cray Programming Environment releases available:
...
## Using non-default versions of HPE Cray libraries on Tursa
...
```
auser@login01:~/test/libsci> module swap cray-libsci cray-libsci/20.08.1.2 
auser@login01:~/test/libsci> export LD_LIBRARY_PATH=$CRAY_LD_LIBRARY_PATH:$LD_LIBRARY_PATH
```
...
```
auser@login01:~/test/libsci> ldd dgemv.x 
...
## Build instructions for software on Tursa

The Tursa CSE team at [EPCC](https://www.epcc.ed.ac.uk) and other contributors
...
Tursa) in a public Github repository. See:

   - [Build instructions repository](https://www.github.com/HPC-UK/build-instructions)

The repository always welcomes contributions from the Tursa user community.

## Support for building software on Tursa

If you run into issues building software on Tursa or the software you
require is not available then please contact the
[Tursa Service Desk](https://www.tursa.ac.uk/support-access/servicedesk.html)
